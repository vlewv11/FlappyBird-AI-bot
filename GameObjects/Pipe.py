...
class Pipe:
    ...
    # Define desired size for the pipes
    PIPE_WIDTH = 52  # Desired width of the pipe
    PIPE_HEIGHT = 320  # Desired height of the pipe
    ...
    def load_pipe_sprites(self):
        # Load and scale the pipe sprite to the desired size
        self.pipe_sprite = pygame.transform.scale(
            pygame.image.load(os.path.join(config.TEXTURES_DIR, "pipe-green.png")).convert_alpha(),
            (self.PIPE_WIDTH, self.PIPE_HEIGHT)
        )

        # Create the upper pipe by flipping the scaled sprite
